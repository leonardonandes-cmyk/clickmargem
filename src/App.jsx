      ...
      <div ref={detRef} style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 16, padding: "20px 22px", maxWidth: 520 }}>
        ...
        <div style={{ display: "grid", gap: 9 }}>
          ...
          {rows.map((x, i) => {
            ...
            return <div key={i} style={{ display: "flex", alignItems: "center", fontSize: 13, ...bt }}><span style={{ color: "var(--tx2)", flex: 1 }}>{x[0]}</span><span style={{ color: x[2], fontWeight: strong, width: 92, textAlign: "right" }}>{val}</span><span style={{ color: pctColor, fontWeight: strong, width: 52, textAlign: "right", fontSize: 12 }}>{pct.toFixed(1)}%</span></div>;
          })}
        </div>
      </div>
    </>
...
function CryptoChart({ d }) {
  ...
  const up = d.best.lucro >= 0;
  const accent = up ? "#00E39A" : "#FF476F";
  ...
  const ticks = 3;
  return (
    <div style={{ background: "linear-gradient(180deg,#0E1421,#0B1019)", border: "1px solid #1E2838", borderRadius: 16, padding: "16px 16px 8px", color: "#E7EDF5", overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
        ...
        <span style={{ marginLeft: "auto", fontSize: 10, fontWeight: 600, letterSpacing: ".12em", color: "#8A94A6", fontFamily: MONO }}>LUCRO · BRL</span>
      </div>
      <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginBottom: 2, flexWrap: "wrap" }}>
        <span style={{ fontSize: 26, fontWeight: 700, fontFamily: MONO, letterSpacing: "-.02em" }}>{money(d.best.lucro)}</span>
        <span style={{ fontSize: 12, fontWeight: 600, color: accent, background: `${accent}22`, padding: "4px 8px", borderRadius: 7, fontFamily: MONO }}>▲ +{money(d.gap)}</span>
      </div>
      <div style={{ fontSize: 11, color: "#8A94A6", marginBottom: 6 }}>Melhor <b style={{ color: "#E7EDF5" }}>{d.best.c.nome}</b> · pior <b style={{ color: "#E7EDF5" }}>{d.worst.c.nome}</b></div>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ display: "block", width: "100%", height: "auto", overflow: "visible" }}>
        ...
        {arr.map((r, i) => (
          <g key={r.c.id}>
            <circle cx={X(i)} cy={Y(r.lucro)} r="5.5" fill="#0E1421" stroke={r.c.cor} strokeWidth="2.4" filter="url(#cgGlow)" />
            <circle cx={X(i)} cy={Y(r.lucro)} r="2.2" fill={r.c.cor} />
            <text x={X(i)} y={H - 9} textAnchor="middle" fontSize="9" fontWeight="600" fill="#8A94A6" fontFamily="Inter,system-ui,sans-serif">{r.c.nome.split(" ")[0]}</text>
          </g>
        ))}
      </svg>
    </div>
